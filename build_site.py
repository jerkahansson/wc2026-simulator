...
(function () {
  ...
  // LAYER_CAPS[1] (3rd layer) → 4 then 4×3 = 12 nodes. Tune here.
  const LAYER_CAPS = [4, 3]; // children per node, by depth (0 = root)
  ...
  function buildTree(anim, wrap, M, st) {
    ...
    for (let i = 0; i < flat.length; i++) {
      ...
      if (n.kind === 'champ' || n.kind === 'lose') {
        ...
        // Node = a win/lose donut for THIS match. Ring: green = win chance,
        // muted red = lose. Size still encodes how likely we are to reach the
        // node (sqrt of its conditional reach prob); capped to the row pitch.
        let r = Math.max(20, 18 + 44 * Math.sqrt(n.cond));
        r = Math.min(r, maxRForDepth(n.depth));
        const lab = nodeLabel(n.last, M);
        const teamCol = n.last.round <= 0 ? '#1f7fc4' : n.last.round === 6 ? '#FECC00' : oppColor(n.last.opp.name);
        const isRoot = n.depth === 0;
        // win chance of the match at this node (group decider/knockouts root = advance)
        const winP = n.last.round <= 0 ? M.advance : beatOf(M, n.last.round - 1, n.last.opp.name);
        const clickable = n.depth > 0;
        const deg = Math.round(winP * 360);
        const oppName = n.last.round <= 0 ? (M.remainingOpponent || 'group') : n.last.opp.name;
        const tip = state.country + ' vs ' + oppName + ' — win ' + pct(winP) + ', lose ' + pct(1 - winP);
        const hole = r * 0.62;            // donut hole radius
        const wfs = Math.max(9, hole * 0.7);   // win% font
        ...
        holeKids.push(el('div', { style: { fontFamily: "'Archivo',sans-serif", fontWeight: '800', fontSize: wfs + 'px', marginTop: '1px', color: '#eaf1fb', lineHeight: '1' }, text: pct(winP) }));
        const node = el('div', {
          title: tip,
          ...
          style: {
            ...
            background: 'conic-gradient(#2ea043 0deg ' + deg + 'deg, #7c3a44 ' + deg + 'deg 360deg)',
